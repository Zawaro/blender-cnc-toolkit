...
def create_compositor(context, props):
  ...
  if render_type == "DEFAULT":
    ...
    _wire_shadow(context, node_tree, rl, denoise, go, links, nodes, props, config)
...
def _create_remap_hue(context, nodes, links, rl_node, go, props):
  ...
  hue_mix.inputs[2].default_value = (*props.remap_color, 1)
...
def _wire_preview(context, tree, rl, denoise, go, links, nodes, props, config):
  ...
  if use_aa_bypass:
    if props.engine == "CYCLES":
      ...
      links.new(crypto.outputs[1], opacity.inputs[0])
    ...
    if props.engine == "CYCLES":
      links.new(threshold.outputs[0], ao_tint.inputs[0])
    else:
      links.new(crypto.outputs[1], ao_tint.inputs[0])
    ...
    if hue_mix:
      ...
      links.new(denoise.outputs[0], ao_tint.inputs[1])
...
def _wire_shadow(context, tree, rl, denoise, go, links, nodes, props, config):
  bg_rgb = nodes.new("CompositorNodeRGB")
  bg_rgb.name = f"{PREFIX}BackgroundRGB"
  if props.transparent_bg:
    bg_rgb.outputs[0].default_value = (0, 0, 0, 0)
...
def update_scene_params(context):
  ...
  if bg:
    if props.transparent_bg:
      ...
      bg.outputs[0].default_value = (*props.background_color, 1)
  sh = tree.nodes.get(f"{PREFIX}ShadowRGB")
  if sh:
    sh.outputs[0].default_value = (*props.shadow_color, 1)
