...
class CropCanvasProperties(bpy.types.PropertyGroup):
  use_crop_canvas: bpy.props.BoolProperty(
    name="Use Crop Canvas",
    description="Enable cropped render border",
    default=False,
    update=lambda self, ctx: self.update_resolution(ctx),
  )
...
class RENDER_PT_crop_canvas_format(bpy.types.Panel):
  bl_idname = "RENDER_PT_cc_crop_canvas"
