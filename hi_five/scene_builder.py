...
def create_planes(context):
  ...
  _make_plane(f"{PREFIX}Plane.shadow2", hide_render=True, hide_viewport=True, shadow_catcher=True, material=shadow)
  ...
  _make_plane(f"{PREFIX}Plane.shadow", hide_render=True, hide_viewport=True, shadow_catcher=True, material=shadow)
...
def _wire_shadow(tree, rl, go, links, nodes, props, config):
  bg_rgb = nodes.new("CompositorNodeRGB")
  bg_rgb.name = f"{PREFIX}BackgroundRGB"
  bg_rgb.outputs[0].default_value = props.shadow_color
  bg_rgb.location = (0, 200)
  ...
  cr.color_ramp.elements[0].color = config.colorramp_color01
  cr.color_ramp.elements[1].position = config.colorramp_position02
  cr.color_ramp.elements[1].color = config.colorramp_color02
